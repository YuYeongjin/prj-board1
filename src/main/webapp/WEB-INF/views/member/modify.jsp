    ...
    <div class="col">
      ...
      <form id="form1" action="" method="post">
        ...
        <div class="mb-3">
          ...
          <div id="emailText1"class="form-text">이메일 중복확인을 하세요.</div>
        </div>
        <div class="mb-3">
          <label for="" class="form-label">
            가입일시
...
<script>
  ...
  function enableModifyButton() {
    const modify = document.querySelector("#modifyButton1");
        if(availableEmail&&availablePassword&&availableNickName){
  ...
  document.querySelector("#passwordInput2").addEventListener("keyup", function(){
    availablePassword=false;
    enableModifyButton();
  });
  ...
  document.querySelector("#nickNameButton1").addEventListener("click", function () {
    availableNickName = false;
  ...
  document.querySelector("#emailButton1").addEventListener("click", function() {
    availableEmail = false;
    ...
    fetch(ctx+"/member/existEmail",{
        ...
        body: JSON.stringify({email})
            ...
            .then(res=>res.json())
            .then(data=>{document.querySelector("#emailText1").innerText=data.message;
            if(data.status=="not exist"){
              availableEmail=true;
              enableModifyButton();
            }});
